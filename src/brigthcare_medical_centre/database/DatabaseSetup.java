...
public class DatabaseSetup {
    ...
    private static void createTables() throws Exception {
        ...
        if (!tableExists("RECEPTIONISTS")) {
            stmt.execute("CREATE TABLE RECEPTIONISTS ("
                    + "ReceptionistID INT PRIMARY KEY GENERATED ALWAYS AS IDENTITY, "
                    + "Username VARCHAR(50) UNIQUE NOT NULL, "
                    + "FullName VARCHAR(100) NOT NULL, "
                    + "ContactNumber VARCHAR(20), "
                    + "FOREIGN KEY (Username) REFERENCES USERS(Username))");
            System.out.println("Created table: RECEPTIONISTS");
        }
    ...
    private static void seedTestData() throws Exception {
        ...
        // Add test receptionist
        if (!userExists(conn, "receptionist1")) {
            try (PreparedStatement insert = conn.prepareStatement(
                    "INSERT INTO USERS (Username, PasswordHash, Role) VALUES (?, ?, ?)");
                 PreparedStatement insertRec = conn.prepareStatement(
                    "INSERT INTO RECEPTIONISTS (Username, FullName, ContactNumber) VALUES (?, ?, ?)")) {
            insert.setString(1, "receptionist1");
            insert.setString(2, hashPassword("receptionist123")); 
            insert.setString(3, "RECEPTIONIST");
            insert.executeUpdate();
            
            insertRec.setString(1, "receptionist1");
            insertRec.setString(2, "Sarah Reception");
            insertRec.setString(3, "0198765432");
            insertRec.executeUpdate();
            }

            System.out.println("Seeded test receptionist: receptionist1 / receptionist123");
        }
