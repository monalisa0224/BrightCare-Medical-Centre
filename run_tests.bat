...
set HAS_JUNIT=0
if exist "%JUNIT_LIB%" if exist "%HAMCREST_LIB%" set HAS_JUNIT=1
...
if "!HAS_JUNIT!"=="1" (
    ...
    javac -source 1.8 -target 1.8 -cp "!LIBS!;%CLASSES%" -d "%TEST_CLASSES%" !TEST_FILES!
    if errorlevel 1 exit /b 1
    ...
    javac -source 1.8 -target 1.8 -cp "!LIBS!;%CLASSES%" -d "%TEST_CLASSES%" test\brigthcare_medical_centre\tests\RegressionSmokeTests.java
    if errorlevel 1 exit /b 1
)
